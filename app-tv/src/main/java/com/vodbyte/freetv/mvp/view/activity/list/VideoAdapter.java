package com.vodbyte.freetv.mvp.view.activity.list;
...
import static com.vodbyte.freetv.app.Constant.VIDEO_LIST_COLUMN;
...
public class VideoAdapter extends RecyclerView.Adapter<VideoAdapter.ViewHolder> {
    ...
    private ArrayList<VodBean> videoList;
    ...
    public VideoAdapter(Context context, OnClickListener listener, ArrayList<VodBean> videoList, int leftFocus, OnStartLoadMoreListener onStartLoadMoreListener) {
    ...
    void setList(ArrayList<VodBean> videoList) {
        this.videoList = videoList;
    }

    ArrayList<VodBean> getList() {
        return videoList;
    }

    void refreshList(ArrayList<VodBean> videoList, int leftFocus) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.cell_vod_item, parent, false);
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        Glide.with(context).load(videoList.get(position).getVod_pic()).skipMemoryCache(true).diskCacheStrategy(DiskCacheStrategy.RESOURCE).into(holder.video_item_iv_image);
        holder.video_item_tv_title.setText(videoList.get(position).getVod_name());
        if (leftFocus != -1) {
